    ...
    public static IEndpointRouteBuilder MapWorkflowEndpoints(this IEndpointRouteBuilder endpoint)
    {
        ...
        workflow.MapPost("",
            ...
            async (IWorkflowService service, CreateWorkflowDto request) => 
                await service.CreateWorkflowAsync(request.Name, request.Definition, request.WorkspaceId, request.Description, request.Tags, request.AgentId));

        workflow.MapPut("{id}",
            [EndpointSummary("更新工作流"), EndpointDescription("更新工作流")]
            async (IWorkflowService service, long id, UpdateWorkflowDto request) => 
                await service.UpdateWorkflowAsync(id, request.Name, request.Definition, request.Description, request.Tags));
